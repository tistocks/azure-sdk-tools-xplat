...
exports.launchBrowser = function (url, cb) {
  resolveUrl(url, 1, function (url) {
...
var resolveUrl = exports.resolveUrl = function (targetUrl, levelLimit, cb) {
  function processResponse (rsp) {
    if (rsp.statusCode === 302) {
      targetUrl = rsp.headers.location;
      resolveUrl(targetUrl, levelLimit, cb);
    } else {
      cb(targetUrl);
    }
  }
  ...
  if (levelLimit === 0) {
    ...
    if (levelLimit !== -1) {
      ...
      throw new Error('Level limit has to be a number larger than 0, or -1 if no limit applies.');
    }

    var parsedUrl = url.parse(targetUrl);
    if (parsedUrl.protocol === 'http:') {
      http.get(targetUrl, processResponse);
    } else {
      https.get(targetUrl, processResponse);
    }
  }
};